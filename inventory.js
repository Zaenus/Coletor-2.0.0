...
async function searchProduct() {
    ...
    try {
        const response = await fetch(`http://localhost:3000/api/searchProduct?code=${productCode}`);
...
async function registerInventory() {
    try {
        ...
        if (!isNaN(quantidadeContada) && quantidadeContada > 0) {
            const response = await fetch('http://localhost:3000/api/registerInventory', {
                method: 'POST',
                headers: {
...
function updateInventoryDisplay(quantidade_contada, codigo_de_barras, produto, data_hora, codigo) {
    ...
    inventoryRemov.classList.add('inventory-remov');
        ...
        removeButton.addEventListener('click', async () => {
            try {
                const response = await fetch(`http://localhost:3000/api/removeInventoryItem/${codigo}/${data_hora}`, {
                    method: 'DELETE',
                });
    ...
    const updateQuantity = async () => {
        ...
        if (!isNaN(newQuantity) && newQuantity >= 0) {
            try {
                ...
                const response = await fetch(`http://localhost:3000/api/updateInventoryQuantity/${codigo}/${data_hora}`, {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json',
                    },
...
async function fetchInventoryCompactionData() {
    try {
        const response = await fetch('http://localhost:3000/api/inventoryCompaction');
...
async function fetchTotalQuantityCounted(codigo) {
    try {
        ...
        const response = await fetch(`http://localhost:3000/api/totalQuantityCounted/${codigo}`);
        const result = await response.json();
